...
when not(defined(MINIZ_NO_ZLIB_APIS)):
  ...
  proc mz_deflateInit*(pStream: mz_streamp; level: cint): cint {.importc.}
  ...
  proc mz_deflate*(pStream: mz_streamp; flush: cint): cint {.importc.}
  # mz_deflateEnd() deinitializes a compressor:
  # Return values:
  #  MZ_OK on success.
  #  MZ_STREAM_ERROR if the stream is bogus.
  proc mz_deflateEnd*(pStream: mz_streamp): cint {.importc.}
  ...
  proc mz_inflateInit*(pStream: mz_streamp): cint {.importc.}
  # mz_inflateInit2() is like mz_inflateInit() with an additional option that controls the window size and whether or not the stream has been wrapped with a zlib header/footer:
  # window_bits must be MZ_DEFAULT_WINDOW_BITS (to parse zlib header/footer) or -MZ_DEFAULT_WINDOW_BITS (raw deflate).
  proc mz_inflateInit2*(pStream: mz_streamp; window_bits: cint): cint {.importc.}
  ...
  proc mz_inflate*(pStream: mz_streamp; flush: cint): cint {.importc.}
